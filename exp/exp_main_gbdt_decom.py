...
class Exp_Main_GBDT_Decom(Exp_Basic):
    def __init__(self, args):
        super(Exp_Main_GBDT_Decom, self).__init__(args)
    ...
    def get_data_for_gbdt(self, data_loader):
        ...
        if self.args.features == 'S':
            ...
            if self.args.channel_strategy == "CF":
                ...
                # 然后再合并前两维x
                # 所以最后合并后的shape为[samples*channel, seq_len]
    ...
    def add_features(self, data_x_train, data_x_vali, data_x_test, data_x_mark_train, data_x_mark_vali, data_x_mark_test, s_or_t):
        data_x_new_total = {}
        phase_current = 0
        feature_names = []
        for flag in ["train", "vali", "test"]:
            ...
            if flag == "train": 
                if s_or_t == "season": revin_model = self.revin_train_season
                elif s_or_t == "trend": revin_model = self.revin_train_trend
                data_x = data_x_train; data_x_mark = data_x_mark_train
            elif flag == "vali":
                if s_or_t == "season": revin_model = self.revin_vali_season
                elif s_or_t == "trend": revin_model = self.revin_vali_trend
                data_x = data_x_vali; data_x_mark = data_x_mark_vali
            elif flag == "test": 
                if s_or_t == "season": revin_model = self.revin_test_season
                elif s_or_t == "trend": revin_model = self.revin_test_trend
                data_x = data_x_test; data_x_mark = data_x_mark_test
    ...
    def train(self, training):
        ...
        class series_decomp(nn.Module):
            ...
            def forward(self, x):
                ...
                res = x - moving_mean
                if isinstance(x, np.ndarray):
                    moving_mean = moving_mean.detach().numpy()
        ...
        # 如果策略为CI_one，那么x的shaper为：[Batch*Channel, Input length]
        train_x_season, train_x_trend = self.decompsition(train_x)
        train_y_season, train_y_trend = self.decompsition(train_y)
        vali_x_season, vali_x_trend = self.decompsition(vali_x)
        vali_y_season, vali_y_trend = self.decompsition(vali_y)
        test_x_season, test_x_trend = self.decompsition(test_x)
        test_y_season, test_y_trend = self.decompsition(test_y)
        ...
        if add_revin:
            ...
            if self.args.features == 'S':
                # 事实上，这个时候的enc_in就等于1
                self.revin_train_season = RevIN(self.args.enc_in)
                self.revin_train_trend = RevIN(self.args.enc_in)
                self.revin_vali_season = RevIN(self.args.enc_in)
                self.revin_vali_trend = RevIN(self.args.enc_in)
                self.revin_test_season = RevIN(self.args.enc_in)
                self.revin_test_trend = RevIN(self.args.enc_in)
                ...
                self.revin_train_season = RevIN(num_features=1)
                self.revin_train_trend = RevIN(num_features=1)
                self.revin_vali_season = RevIN(num_features=1)
                self.revin_vali_trend = RevIN(num_features=1)
                self.revin_test_season = RevIN(num_features=1)
                self.revin_test_trend = RevIN(num_features=1)
            ...
            # seasonal
            train_x_after_season, vali_x_after_season, test_x_after_season, total_feature_nums, discrete_feature_nums, feature_names = \
                self.add_features(train_x, vali_x, test_x, train_x_mark, vali_x_mark, test_x_mark, s_or_t="season")
            print(type(train_x_after_season), train_x_after_season.shape)
            # trend
            train_x_after_trend, vali_x_after_trend, test_x_after_trend, total_feature_nums, discrete_feature_nums, feature_names = \
                self.add_features(train_x, vali_x, test_x, train_x_mark, vali_x_mark, test_x_mark, s_or_t="trend")
            print(type(train_x_after_trend), train_x_after_trend.shape)
            
            # print(train_x)
        else:
            # 不做revin的话就不加特征
            train_x_after_season = train_x_season.copy()
            vali_x_after_season = vali_x_season.copy()
            test_x_after_season = test_x_season.copy()
            
            train_x_after_trend = train_x_trend.copy()
            vali_x_after_trend = vali_x_trend.copy()
            test_x_after_trend = test_x_trend.copy()
        
        
        train_mse = 10000  # 先在外面定义一个train_mse
        def custom_mse_after_revin_season(y_true: np.ndarray, y_pred: np.ndarray) -> np.ndarray:
            sample_num = y_pred.shape[0]
            seq_len = y_pred.shape[1]
            # print(sample_num, seq_len)
            
            # print("y_pred before revin", y_pred)
            y_pred = self.revin_train_season(torch.from_numpy(y_pred), "denorm")
        ...
        def custom_mse_after_revin_trend(y_true: np.ndarray, y_pred: np.ndarray) -> np.ndarray:
            sample_num = y_pred.shape[0]
            seq_len = y_pred.shape[1]
            # print(sample_num, seq_len)
            
            # print("y_pred before revin", y_pred)
            y_pred = self.revin_train_trend(torch.from_numpy(y_pred), "denorm")
            y_pred = y_pred.detach().numpy()
            # print(type(y_pred), y_pred.shape)
            # print("y_pred after revin", y_pred)
        ...
        def custom_mse_vali_after_revin_season(y_true, y_pred):
            y_true = y_true.reshape(*y_pred.shape)
            # print(y_pred.shape, y_true.shape)
            
            # print("y_pred before revin", y_pred)
            y_pred = self.revin_vali_season(torch.from_numpy(y_pred), "denorm")
            y_pred = y_pred.detach().numpy()
            # print(type(y_pred), y_pred.shape)
            # print("y_pred after revin", y_pred)
        ...
        def custom_mse_vali_after_revin_trend(y_true, y_pred):
            y_true = y_true.reshape(*y_pred.shape)
            # print(y_pred.shape, y_true.shape)
            
            # print("y_pred before revin", y_pred)
            y_pred = self.revin_vali_trend(torch.from_numpy(y_pred), "denorm")
            y_pred = y_pred.detach().numpy()
            # print(type(y_pred), y_pred.shape)
            # print("y_pred after revin", y_pred)
        ...
        model_season = xgb.XGBRegressor(
                        ...
                        objective=custom_mse_after_revin_season,
                        # eval_metric=custom_mse_vali,
                        # eval_metric=custom_eval_metric,
                        eval_metric=custom_mse_vali_after_revin_season,
        ...
        model_trend = xgb.XGBRegressor(
                        ...
                        # objective=custom_mse,
                        # objective=custom_obj,
                        objective=custom_mse_after_revin_trend,
                        # eval_metric=custom_mse_vali,
                        # eval_metric=custom_eval_metric,
                        eval_metric=custom_mse_vali_after_revin_trend,
                        booster='gbtree',         # 有两种模型可以选择gbtree和gblinear。gbtree使用基于树的模型进行提升计算，gblinear使用线性模型进行提升计算。默认为gbtree
                        gamma=0,                 # 叶节点上进行进一步分裂所需的最小"损失减少"。默认0；
                        min_child_weight=1,      # 可以理解为叶子节点最小样本数，默认1；
                        subsample=1,              # 训练集抽样比例，每次拟合一棵树之前，都会进行该抽样步骤。默认1，取值范围(0, 1]
        ...
        print("train_x_after_season.shape:", train_x_after_season.shape)
        print("train_x_after_trend.shape:", train_x_after_trend.shape)
        print("train_y_season.shape:", train_y_season.shape)
        print("train_y_trend.shape:", train_y_trend.shape)
        
        grid_search = False
        if grid_search:
            optimized_model = GridSearchCV(estimator=model_season, param_grid=cv_params, verbose=2, n_jobs=-1, scoring="neg_mean_squared_error")
            optimized_model = GridSearchCV(estimator=model_trend, param_grid=cv_params, verbose=2, n_jobs=-1, scoring="neg_mean_squared_error")
            ...
            model_season.fit(train_x_after_season, train_y_season, eval_set=[(vali_x_after_season, vali_y_season)], early_stopping_rounds=20, verbose=True)
            model_trend.fit(train_x_after_trend, train_y_trend, eval_set=[(vali_x_after_trend, vali_y_trend)], early_stopping_rounds=20, verbose=True)
        ...
        model_season.get_booster().feature_names = feature_names
        model_trend.get_booster().feature_names = feature_names
        ...
        best_score_season = model_season.best_score
        best_score_trend = model_trend.best_score
        
        # print("rmse: ", best_score)
        # print("mse: ", best_score**2)
        
        print()
        print("best score_season on vali: ", best_score_season)
        print("best score_trend on vali: ", best_score_trend)
        print()
        
        # val_mse = best_score**2
        val_mse_season = best_score_season
        val_mse_trend = best_score_trend
        ...
        # 自定义后处理函数
        def post_processing(predictions, c_or_t): 
            if c_or_t == "season":
                processed_predictions = self.revin_test_season(torch.from_numpy(predictions), "denorm")
            elif c_or_t == "trend":
                processed_predictions = self.revin_test_trend(torch.from_numpy(predictions), "denorm")
            processed_predictions = processed_predictions.detach().numpy()
            return processed_predictions
        
        pred_y_season = model_season.predict(test_x_after_season)
        pred_y_trend = model_trend.predict(test_x_after_trend)
        # 如果做了revin，别忘记denorm回来
        if add_revin:
            pred_y_season = post_processing(pred_y_season, c_or_t="season")
            pred_y_trend = post_processing(pred_y_trend, c_or_t="trend")
        print("pred_y_season.shape: ", pred_y_season.shape)
        print(pred_y_season)
        print("pred_y_trend.shape: ", pred_y_trend.shape)
        print(pred_y_trend)
        
        # 把season和trend分量加回来就得到最后的预测的y值了
        pred_y = pred_y_season + pred_y_trend
        ...
        print("MSE on test_season:", mean_squared_error(test_y_season, pred_y_season))
        print("MAE on test_season:", mean_absolute_error(test_y_season, pred_y_season))
        print("MSE on test_trend:", mean_squared_error(test_y_trend, pred_y_trend))
        print("MAE on test_trend:", mean_absolute_error(test_y_trend, pred_y_trend))
        ...
        feature_importance_weight_season = model_season.get_booster().get_score(importance_type='weight')
        print("features length: ", len(feature_importance_weight_season.items()))
        tmp = sorted(feature_importance_weight_season.items(), key=lambda d: d[1], reverse=True)
        # print(tmp[:10])
        print("season:", tmp)
        feature_importance_gain_season = model_season.get_booster().get_score(importance_type='gain')
        tmp = sorted(feature_importance_gain_season.items(), key=lambda d: d[1], reverse=True)
        # print(tmp[:10])
        print("season:", tmp)
        
        feature_importance_weight_trend = model_trend.get_booster().get_score(importance_type='weight')
        print("features length: ", len(feature_importance_weight_trend.items()))
        tmp = sorted(feature_importance_weight_trend.items(), key=lambda d: d[1], reverse=True)
        # print(tmp[:10])
        print("trend:", tmp)
        feature_importance_gain_trend = model_trend.get_booster().get_score(importance_type='gain')
        tmp = sorted(feature_importance_gain_trend.items(), key=lambda d: d[1], reverse=True)
        # print(tmp[:10])
        print("trend:", tmp)
        ...
        return train_mse, val_mse_season, val_mse_trend, test_mse, test_mae
